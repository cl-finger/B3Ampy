...
def extrema(P_norm,threshold,dist_threshold,no_maxima):
    ...
    dist_max = np.zeros((xmax.size,ymax.size))
    for ii in range(xmax.size):
        for jj in range(ymax.size):
            dist_max[ii,jj] = np.sqrt((xmax[ii]-xmax[jj])**2+(ymax[ii]-ymax[jj])**2)
    #find if points are seperate focus points
    for ii in range(xmax.size):
        for jj in range(ymax.size):
            if jj > ii: #only upper triangle
                if dist_max[ii,jj]<dist_threshold:
